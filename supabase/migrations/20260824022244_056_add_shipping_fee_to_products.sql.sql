...
-- Update the create product RPC to accept shipping_fee
...
BEGIN
  ...
  IF v_profile.membership_tier = 'free' OR v_profile.membership_tier IS NULL THEN
    ...
    END IF;
    -- Check max 5 active direct products
...
REVOKE EXECUTE ON FUNCTION public.rpc_seller_create_product_v2(text, text, text, text, timestamptz, numeric, boolean, numeric, int, numeric) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_seller_create_product_v2(text, text, text, text, timestamptz, numeric, boolean, numeric, int, numeric) TO authenticated;
